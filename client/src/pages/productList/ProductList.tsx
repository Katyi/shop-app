...
const ProductList: React.FC = () => {
  ...
  const pathCat = location.pathname.split('/')[2];
  const cat = queryCat || pathCat || 'all';
  ...
  React.useEffect(() => {
    setFilters({});
    // Если нужно сбрасывать и сортировку на "Новинки", раскомментируй строку ниже:
    setSort(SORT_KEYS.NEWEST);
  }, [cat]);
  ...
  //   if (cat === 'all') return t('productList.allCat');

  //   // Ищем точное совпадение (например "women,socks")
  //   const currentCat = categories.find((c) => c.cat === cat);
  //   if (currentCat) return currentCat.title;

  //   // Если не нашли (например, системный сбой), пробуем перевести последний сегмент
  //   const lastPart = cat.split(',').pop();
  //   return t(`categories.${lastPart}`, { defaultValue: cat });
  // };
  ...
  return (
    <div className="pageContainer flex-col">
      ...
      {/* Контейнер фильтров */}
      <div className="sticky top-27 z-50 bg-white pb-8 flex flex-col md:flex-row justify-between px-5 gap-4">
        {/* Левый блок: Фильтры */}
